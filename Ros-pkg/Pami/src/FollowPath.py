...
def follow_path(pose_pub, parent_frame, threshold=0.05):
    ...
    for point in path_points:
        ...
        if dist < min_distance:
            min_distance = dist
            nearest_point = point
    index = path_points.index(nearest_point)
    nearest_point = path_points[index+1]
    ...
    for point in path_points[start_index:]:
        ...
        while True:
            ...
            if current_pos is None or distance(current_pos, point) < threshold or (time.time()-a)>1:
                #if booltest == True:
                #    return
                #booltest = True
                break
    while True:
        send_to_robot(pose_pub, (0,0))
