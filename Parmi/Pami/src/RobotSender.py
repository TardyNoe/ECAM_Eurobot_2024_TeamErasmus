...
def sendToRobot(angleGoal,power):	
    ...
    message = byte_array + int(min(300*power,150)).to_bytes(1,byteorder='big')
    sock.sendto(message, (esp_ip, esp_port))

def exctractData(data):
    try:
        x,y,theta,distance = struct.unpack('hhhb',data)
        angle = -math.radians(theta/10)
        posx = (x/1000)*(0.7/0.98)
        posy = (-y/1000)*(0.7/0.98)
        return posx,posy,angle,distance
...
def computeCommand(posx,posy,xgoal,ygoal,angleGoal,angle):
    ...
    if abs(angleGoal-math.degrees(angle)-180)>15:
        power = 0
...
if __name__ == '__main__':
    ...
    while not rospy.is_shutdown():
        ...
        try:
            ...
            posx,posy,angle,distance = exctractData(data)	
